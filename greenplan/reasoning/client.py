...
class ReasoningModel:
    ...
    def recommend(
        ...
        for i in range(0, len(ranked_rows), size):
            chunk = ranked_rows[i : i + size]
            log.info(
                "recommend: zones %d-%d of %d",
                i + 1, min(i + size, len(ranked_rows)), len(ranked_rows),
            )
            try:
                out.extend(self._recommend_chunk(chunk, lessons))
            except OpenRouterError as exc:
                # The run must never die because prose formatting failed.
                # Degrade transparently: the deterministic writer produces the
                # numeric justification and heuristic species for this chunk,
                # and the log says so out loud.
                log.warning(
                    "LLM never produced valid JSON for zones %d-%d (%s) — "
                    "falling back to the deterministic writer for them",
                    i + 1, min(i + size, len(ranked_rows)), exc,
                )
                out.extend(MockModel().recommend(chunk, lessons))
        return out
...
def build_model(cfg: ModelCfg, mock: bool) -> Any:
    ...
    if cfg.provider == "openvino":
        ...
        model.chunk_zones = 2  # small local model: short JSON replies only
        return model
    if cfg.provider == "hybrid":
        ...
        llm.chunk_zones = 2  # small local model: short JSON replies only
